...
export default function EditDevisPage() {
  ...
  const fetchDevis = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          clientId: devis.clientId._id || devis.clientId,
          chantierName: devis.chantierName,
          location: devis.location,
          tvaRate: devis.tvaRate,
          notes: devis.notes || '',
          validUntil: devis.validUntil ? new Date(devis.validUntil).toISOString().split('T')[0] : '',
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.clientId || !formData.chantierName || !formData.location) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.error || 'Erreur lors de la modification du devis');
      }
    } catch (error) {
      toast.error('Erreur de connexion');
    } finally {
