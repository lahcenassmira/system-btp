...
export default function InvoicesPage() {
  ...
  const handleDownloadPDF = async (invoiceId: string, invoiceNumber: string) => {
    try {
      const response = await authenticatedFetch(`/api/invoices/${invoiceId}/pdf?lang=${locale}`);
  ...
  const handleSendEmail = async (invoice: Invoice) => {
    try {
      const response = await authenticatedFetch(`/api/invoices/${invoice._id}/email`, {
        method: 'POST',
        body: JSON.stringify({
          language: locale,
          sendCopy: false
  ...
  const handleSendReminder = async (invoice: Invoice) => {
    try {
      const response = await authenticatedFetch(`/api/invoices/${invoice._id}/reminder`, {
        method: 'POST'
      });
      if (!response.ok) {
